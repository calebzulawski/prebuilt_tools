...
def strip_noarch(platforms: Sequence[str]) -> List[str]:
    return [platform for platform in platforms if platform != "noarch"]
...
def write_env_file(
    ...
    *,
    ...
    lines = [
        ...
        "  - conda-forge",
        "dependencies:",
        f"  - {dependency_constraint(package_name, minor_version)}",
    ]
    if platforms:
...
def write_generated_module(repo_root: Path, environment_names: Sequence[str]) -> None:
...
def write_tools_build_files(
    ...
    for tool_name, version_map in sorted(tool_versions.items()):
        ...
        for minor_version, environment_name in versions:
            ...
            lines.append(f'    name = "{minor_version}",')
            lines.append(f'    environment = "@{environment_name}",')
            lines.append(
                "    path = select({"
            )
            lines.append(
                f'        "@platforms//os:windows": "Library/bin/{tool_name}",'
            )
            lines.append(
                f'        "//conditions:default": "bin/{tool_name}",'
            )
            lines.append("    }),")
        ...
        if versions:
            ...
            lines.append(f'    actual = ":{latest_version}",')
            lines.append('    visibility = ["//visibility:public"],')
            lines.append(")")
            lines.append("")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    packages_to_query: List[str] = []
    for spec in package_specs:
        packages_to_query.append(spec.package)
        if spec.win_package:
            packages_to_query.append(spec.win_package)

    availability_by_package = asyncio.run(
        resolve_availability(channel, DEFAULT_SUBDIRS, packages_to_query, gateway)
    )

    platform_objs = {name: Platform(name) for name in DEFAULT_SUBDIRS}
    windows_platforms = {
        name
        for name, platform in platform_objs.items()
        if platform.is_windows and name not in SKIP_PLATFORMS
    }
    all_platforms = set(DEFAULT_SUBDIRS)
    non_windows_platforms = all_platforms - windows_platforms
    noarch_platforms = {name for name in all_platforms if name == "noarch"}
    windows_with_noarch = windows_platforms | noarch_platforms
    ...
    for spec in package_specs:
        ...
        for minor_version, platforms in filter_availability(base_availability, allowed_base).items():
            non_noarch = strip_noarch(platforms)
            filtered_base[minor_version] = non_noarch
        ...
        for minor_version, platforms in list(filtered_base.items()):
            ...
            platform_candidates = sorted(
                p for p in allowed_base if p not in SKIP_PLATFORMS and p != "noarch"
            )
        ...
        if spec.win_package:
            for minor_version, platforms in win_availability.items():
                ...
                generated_files[spec.package].append(env_path)
                environment_names.add(base_env_name)
                for tool in spec.executables:
    ...
    write_generated_module(repo_root, sorted(environment_names))
